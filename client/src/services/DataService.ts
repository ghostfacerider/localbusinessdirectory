...
export const getOnePlayer = async (
  id: string,
  callback: (
    data: businessTypes.Player | null,
...
export const updatePlayer = async (
  ...
  try {
    const response: AxiosResponse<businessTypes.Player> =
      await axiosInstance.put(`$/players/${updateId}`, updatePlayer, {
        headers: { 'x-auth-token': authService.getToken() },
      });
    callback(response.data, null);
  } catch (error) {
